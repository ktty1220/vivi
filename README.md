...
もう一つ、iScrollではスクロールした時に右端にスクロールバーが表示され、コンテンツの長さと現在位置が把握できるようになっていますが、ViViではスクロールバーは実装されていません(今後の実装予定には入っています)。個人的にはそれほど不便はないのですが、これを重視する方もいるかもしれないので一応この点もお知らせしておきます。
...
[Articlip](http://articlip.ktty1220.me/)
...
なお、現時点でHammer.jsの最新バージョンは1.0.10ですが、最新版を使用するとスクロールの動作がおかしくなる現象が確認されています。現在ViViに組み込まれているHammer.jsのバージョンは1.0.6ですが、それ以上のバージョンには上げないようにしてください。
